	...
	<view class="editor-container">
		...
		<view class="fixed-bottom" :hidden="!toolbarShow" :style="{ bottom: fixedBottom + 'px' }" @touchstart.stop="">
			<view class="toolbar selector" :style="{height: toolBarHeight + 'rpx'}">
				<view class="toolbar-item-header" @touchend.stop="changeKeyBoard"><view class="iconfont icon-icon_keyboard "></view>
				</view>
				<view v-for="(icon, index) in ['icon-icon-add', 'icon-icon_font_size', 'icon-icon_align_left']" class="toolbar-item"
					@touchend.stop="changeSwiper(index)">
					<text class="iconfont" :class="[icon, { active: toolBarContentShow && swiperCurrent == index }]"></text>
				</view>

				<view class="toolbar-item" @touchend.stop="formatformat('check')"><i
						class="iconfont icon-icon_check3"></i></view>
				<view class="toolbar-item" @touchend.stop="formatformat('undo')"><i class="iconfont icon-icon-undo"></i>
				</view>
				<view class="toolbar-item" @touchend.stop="formatformat('redo')"><i class="iconfont icon-icon-redo"></i>
				</view>
				<view class="toolbar-item-footer" @touchend.stop="hideToolbar"><i class="iconfont icon-icon_list_check"></i>
				</view>
			</view>
					...
					<template v-if="page.type === 'feature'">
						<view class="feature-items flex">
							<view v-for="(pitem, pindex) in page.array" :key="pindex" class="feature"
								...
								<view class="icon"><text class="iconfont" :class="pitem.icon"></text></view>
							</view>
						</view>
					</template>
					...
					<template v-else-if="page.type === 'tool'">
						<view v-for="(pitem, pindex) in page.array" :key="pindex" class="tool-items flex">
							<view v-for="(item, index) in pitem.items" :key="index" class="tool-item"
								...
								<i v-else-if="pitem.label == 'icon'" class="iconfont" :class="item.icon"></i>
					...
					},
					{
						...
						array: [{
								...
								items: [{
									...
									{
										name: 'underline',
										icon: 'icon-icon_underline'
									},
									{
										name: 'strike',
										icon: 'icon-edui99-icon-strikethrough'
									},
									{
										name: 'backgroundColor',
										value: 'yellow',
										icon: 'icon-zitibeijing'
									}
								]
							},
							{
					...
					},
					{
						...
						array: [{
								...
								items: [{
									...
									{
										value: 'center',
										icon: 'icon-Icon_align_center'
									},
									{
										value: 'right',
										icon: 'icon-Icon_align_right'
									}
								]
							},
							{
								...
								items: [{
									...
									},
									{
										name: 'list',
										value: 'bullet',
										icon: 'icon-icon-unorderedlist-outlined'
									},
									{
										name: 'indent',
										icon: 'icon-iconoutdent',
										value: '+1'
									},
									{
										name: 'indent',
										icon: 'icon-icon-indent',
										value: '-1'
									}
								]
							},
